...
try {
  ...
  assert(missingTokenData.error?.code === 'CSRF_VALIDATION_FAILED', 'Missing-token response should include CSRF error code');
  ...
  assert(invalidOriginData.error?.code === 'CSRF_VALIDATION_FAILED', 'Origin-failure response should include CSRF error code');
