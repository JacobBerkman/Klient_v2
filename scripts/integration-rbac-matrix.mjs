...
try {
  ...
  const checks = [
    ...
    { path: '/api/profiles', method: 'POST', body: { kind: 'prospect', firstName: 'A', lastName: 'B' }, allow: ['admin','advisor'] },
    ...
    { path: '/api/households', method: 'POST', body: { name: 'H', primaryClientId: profile.id }, allow: ['admin','advisor'] },
    { path: `/api/households/${household.id}/members`, method: 'POST', body: { clientId: profile.id, role: 'member' }, allow: ['admin','advisor'] },
    ...
    { path: '/api/forms/submissions', method: 'POST', body: { clientId: profile.id, templateId: formTemplate.id, status: 'draft', data: {} }, allow: ['admin','advisor'] },
    { path: '/api/forms/drafts', method: 'GET', allow: ['admin','advisor','readonly'] },
    { path: `/api/forms/submissions/${submission.id}`, method: 'PATCH', body: { status: 'submitted' }, allow: ['admin','advisor'] },
    ...
    { path: '/api/exports', method: 'POST', body: { clientId: profile.id, templateId: docTemplate.id, type: 'pdf' }, allow: ['admin','advisor'] },
    ...
    { path: '/api/client/forms/submissions', method: 'POST', body: { templateId: formTemplate.id, status: 'draft', data: {} }, allow: ['client'] },
    { path: '/api/client/uploads', method: 'POST', body: { name: 'x' }, allow: ['client'] },
    { path: '/api/portal-links', method: 'POST', body: { profileId: profile.id }, allow: ['admin','advisor','readonly'] },
  ...
  for (const check of checks) {
    for (const [role, token] of Object.entries(actors)) {
      ...
      if (expectation === 'allow') {
        ...
        assert(/Missing permission|Authentication required|not found|CSRF/i.test(denied.message), `${role} must be denied ${method} ${check.path}`);
      }
    }
  }
