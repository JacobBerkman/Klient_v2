...
try {
  ...
  const poisonJob = await context.request('/api/exports', {
    method: 'POST',
    headers,
    body: JSON.stringify({ clientId: profile.id, templateId: template.id, type: 'pdf', metadata: { simulateFailuresRemaining: 9 }, maxAttempts: 8 })
  });
  ...
  assert(poison?.failure?.reason === 'poison_job' || poison?.failure?.reason === 'max_attempts_exhausted', 'Expected actionable dead-letter reason');
