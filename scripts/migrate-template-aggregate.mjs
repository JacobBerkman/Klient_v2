...
function migrate(state) {
  ...
  const legacyIds = new Set([
    ...(state.formTemplates || []).map((entry) => entry.id),
    ...(state.documentTemplates || []).map((entry) => entry.id)
  ])
  ...
  const formTemplates = (state.formTemplates || []).map((template) =>
    normalize(
      {
        ...template,
        blueprint: { sections: [] },
  ...
  const documentTemplates = (state.documentTemplates || []).map((template) =>
    normalize(
      {
        ...template,
