...
export function createS3CompatibleStorageProvider({
  ...
  function signPresigned({ method, bucket, key, expiresInSeconds = 900, extraHeaders = {}, query = {} }) {
    ...
    const canonicalUri = forcePathStyle ? `/${bucket}/${encodePath(String(key || '').replace(/^\/+/, ''))}` : `/${encodePath(String(key || '').replace(/^\/+/, ''))}`;
  ...
  return {
    ...
    async putObject({ bucket, key, body, contentType }) {
      const presigned = signPresigned({ method: 'PUT', bucket, key, extraHeaders: contentType ? { 'content-type': contentType } : {} });
