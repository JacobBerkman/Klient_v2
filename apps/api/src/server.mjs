...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/portal-links' && req.method === 'GET') { const user = requireUser(req); const result = store.listPortalLinks(user, url.searchParams.get('profileId')); finalizeLog(200, { firmId: user.firmId }); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname === '/api/portal-links' && req.method === 'POST') { const body = await parseBody(req); const result = store.createPortalLink(requireUser(req), body); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/portal-links/') && pathname.endsWith('/revoke') && req.method === 'POST') { const id = pathname.split('/')[3]; const result = store.revokePortalLink(requireUser(req), id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
