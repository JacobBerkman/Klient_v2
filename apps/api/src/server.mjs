...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname.startsWith('/api/templates/') && pathname.endsWith('/mappings') && req.method === 'GET') { const id = pathname.split('/')[3]; const result = store.getTemplateMappingEditor(requireUser(req), id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/templates/') && pathname.endsWith('/mappings/suggestions') && req.method === 'POST') { const id = pathname.split('/')[3]; const body = await parseBody(req); const result = store.suggestTemplateMappings(requireUser(req), id, body); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
