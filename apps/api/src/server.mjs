...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/template-engines' && req.method === 'GET') { const result = store.listTemplateEngines(requireUser(req), url.searchParams.get('category') || null); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname === '/api/template-engines' && req.method === 'POST') { const result = store.createTemplateEngine(requireUser(req), await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
