...
export function createHttpServer({ modules }) {
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname === '/api/invites' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canManageUsers'); const result = modules.firmsUsers.inviteUser(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      ...
      if (pathname === '/api/exports' && req.method === 'POST') { const user = requireUser(); modules.policy.requirePermission(user, 'exports:write'); const body = await parseBody(req); const idempotencyKey = req.headers['idempotency-key'] || req.headers['x-idempotency-key'] || body.idempotencyKey; const result = modules.exports.create(user, { ...body, idempotencyKey }); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      ...
      if (pathname === '/api/dashboard' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canViewDashboard'); const result = modules.profiles.getDashboard(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/profiles' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadProfiles'); const result = modules.profiles.listProfiles(user, { kind: url.searchParams.get('kind'), search: url.searchParams.get('search') || '' }); finalizeLog(200, { firmId: user.firmId }); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/profiles' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteProfiles'); const result = modules.profiles.createProfile(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && pathname.endsWith('/stage-history') && req.method === 'GET') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canReadProfiles'); const result = modules.profiles.listStageHistory(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && pathname.endsWith('/notes') && req.method === 'GET') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canReadProfiles'); const result = modules.profiles.listNotes(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && pathname.endsWith('/notes') && req.method === 'POST') { const id = pathname.split('/')[3]; const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canWriteProfiles'); const result = modules.profiles.addNote(user, id, body.body || ''); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && pathname.split('/').length === 4 && req.method === 'GET') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canReadProfiles'); const result = modules.profiles.getProfileDetail(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && pathname.endsWith('/stage') && req.method === 'PATCH') { const id = pathname.split('/')[3]; const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canMovePipeline'); const result = modules.pipeline.moveProfileStage(user, id, body.stage, body.beforeProfileId || null); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && req.method === 'PATCH') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canWriteProfiles'); const result = modules.profiles.updateProfile(user, id, await parseBody(req)); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/board' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadProfiles'); const result = modules.pipeline.getBoard(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/households' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadHouseholds'); const result = modules.households.listHouseholds(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/households' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteHouseholds'); const result = modules.households.createHousehold(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/households/') && pathname.endsWith('/members') && req.method === 'POST') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canWriteHouseholds'); const result = modules.households.addHouseholdMember(user, id, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/households/') && pathname.endsWith('/members') && req.method === 'DELETE') { const id = pathname.split('/')[3]; const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canWriteHouseholds'); const result = modules.households.removeHouseholdMember(user, id, body.clientId); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/households/link-spouse' && req.method === 'POST') { const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canWriteHouseholds'); const result = modules.households.linkSpouse(user, body.primaryClientId, body.spouseClientId); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/households/create-spouse' && req.method === 'POST') { const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canWriteHouseholds'); const result = modules.households.createSpouse(user, body.primaryClientId, body.spouse); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/forms/templates' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadForms'); const result = modules.forms.listFormTemplates(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/forms/templates' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteForms'); const result = modules.forms.createFormTemplate(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/forms/submissions' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadForms'); const result = modules.forms.listFormSubmissions(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/forms/drafts' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadForms'); const result = modules.forms.listFormDrafts(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/forms/submissions' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteForms'); const result = modules.forms.createFormSubmission(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/client/workspace' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadClientWorkspace'); const result = modules.forms.getClientWorkspace(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/client/forms/submissions' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteClientWorkspace'); const result = modules.forms.submitClientForm(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/client/uploads' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteClientWorkspace'); const result = modules.forms.submitClientUpload(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/forms/submissions/') && req.method === 'PATCH') { const id = pathname.split('/')[4]; const user = requireUser(); modules.policy.requireGuard(user, 'canWriteForms'); const result = modules.forms.updateSubmission(user, id, await parseBody(req)); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/forms/submissions/') && req.method === 'DELETE') { const id = pathname.split('/')[4]; const user = requireUser(); modules.policy.requireGuard(user, 'canWriteForms'); const result = modules.forms.deleteSubmission(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/templates' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadTemplate'); const result = modules.templates.list(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/templates' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canEditTemplate'); const result = modules.templates.create(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/templates/auto-build' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canEditTemplate'); const result = modules.templates.autoBuild(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/templates/') && pathname.endsWith('/publish') && req.method === 'POST') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canPublishTemplate'); const result = modules.templates.publish(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/templates/') && pathname.endsWith('/mappings') && req.method === 'POST') { const id = pathname.split('/')[3]; const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canEditTemplate'); const result = modules.templates.updateMappings(user, id, body.mappings || []); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/exports' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadExports'); const result = modules.exports.list(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/exports' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canWriteExports'); const result = modules.exports.create(user, await parseBody(req)); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/exports/process' && req.method === 'POST') { const user = requireUser(); modules.policy.requireGuard(user, 'canProcessExports'); const result = modules.exports.processQueuedExports(user); finalizeLog(200); return json(res, 200, { ...result, deprecated: true, message: 'Manual processing endpoint is deprecated; prefer running scripts/export-worker.mjs.' }, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/exports/') && pathname.endsWith('/retry') && req.method === 'POST') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canWriteExports'); const result = modules.exports.retry(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/audit' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadAudit'); const result = modules.audit.list(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/analytics' && req.method === 'GET') { const user = requireUser(); modules.policy.requireGuard(user, 'canReadAnalytics'); const result = modules.analytics.get(user); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname.startsWith('/api/profiles/') && pathname.endsWith('/sensitive') && req.method === 'GET') { const id = pathname.split('/')[3]; const user = requireUser(); modules.policy.requireGuard(user, 'canReadSensitiveProfileData'); const result = modules.profiles.getMaskedSensitiveData(user, id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
      if (pathname === '/api/portal-links' && req.method === 'POST') { const body = await parseBody(req); const user = requireUser(); modules.policy.requireGuard(user, 'canCreatePortalLink'); const result = modules.forms.createPortalLink(user, body.profileId); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
...
export async function startServer() {
  ...
  try {
    ...
    if (pathname === '/api/exports' && req.method === 'POST') { const body = await parseBody(req); const idempotencyKey = req.headers['idempotency-key'] || req.headers['x-idempotency-key'] || body.idempotencyKey; const result = store.createExport(requireUser(req), { ...body, idempotencyKey }); finalizeLog(201); return json(res, 201, result, { 'X-Request-Id': requestId }); }
    if (pathname === '/api/exports/process' && req.method === 'POST') {
      const user = requireUser(req);
