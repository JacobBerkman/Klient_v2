...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname.startsWith('/api/forms/templates/') && pathname.endsWith('/publish') && req.method === 'POST') { const id = pathname.split('/')[4]; const result = store.publishFormTemplate(requireUser(req), id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/forms/templates/') && pathname.endsWith('/versions') && req.method === 'GET') { const id = pathname.split('/')[4]; const result = store.listFormTemplateVersions(requireUser(req), id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/forms/templates/') && pathname.endsWith('/revert') && req.method === 'POST') { const segments = pathname.split('/'); const id = segments[4]; const version = Number(segments[6]); const result = store.revertFormTemplateVersion(requireUser(req), id, version); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/forms/templates/') && pathname.split('/').length === 5 && req.method === 'PATCH') { const id = pathname.split('/')[4]; const result = store.updateFormTemplate(requireUser(req), id, await parseBody(req)); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    ...
    if (pathname.startsWith('/api/templates/') && pathname.endsWith('/versions') && req.method === 'GET') { const id = pathname.split('/')[3]; const result = store.listDocumentTemplateVersions(requireUser(req), id); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/templates/') && pathname.endsWith('/revert') && req.method === 'POST') { const segments = pathname.split('/'); const id = segments[3]; const version = Number(segments[5]); const result = store.revertTemplateVersion(requireUser(req), id, version); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
    if (pathname.startsWith('/api/templates/') && pathname.split('/').length === 4 && req.method === 'PATCH') { const id = pathname.split('/')[3]; const result = store.updateDocumentTemplate(requireUser(req), id, await parseBody(req)); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
