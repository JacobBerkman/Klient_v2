...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname.startsWith('/api/templates/') && pathname.endsWith('/blueprint') && req.method === 'POST') { const id = pathname.split('/')[3]; const body = await parseBody(req); const result = store.updateTemplateBlueprint(requireUser(req), id, body.blueprint || { sections: [] }); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
