...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname.startsWith('/api/forms/templates/') && req.method === 'PATCH') { const id = pathname.split('/')[4]; const result = store.updateFormTemplate(requireUser(req), id, await parseBody(req)); finalizeLog(200); return json(res, 200, result, { 'X-Request-Id': requestId }); }
