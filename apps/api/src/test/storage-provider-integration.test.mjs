...
function buildMockStorage() {
  ...
  return {
    ...
    provider: {},
    async putObject() { return { etag: 'mock-etag' }; },
    async getObject() { return { body: Buffer.from('mock') }; },
    ...
    async createPresignedDownloadUrl() {
      ...
      return { method: 'GET', url: '/mock/download', headers: {}, expiresAt: new Date(Date.now() + 60000).toISOString() };
    },
    async deleteObject() {
      calls.delete += 1;
    },
...
test('client upload presign stores object metadata via provider contract', async () => {
  ...
  const client = { id: clientUser.id, firmId, email: clientUser.email, firstName: clientUser.firstName, lastName: clientUser.lastName, role: clientUser.role };

  const presign = await store.createClientUploadPresign(client, { fileName: 'tax-return.pdf', contentType: 'application/pdf', category: 'tax' });
  assert.equal(objectStorage.calls.presignUpload, 1);
  assert.equal(presign.object.bucket, 'test-docs');

  const saved = store.submitClientUpload(client, { uploadId: presign.uploadId, name: 'Tax Return 2025', category: 'tax', object: presign.object });
...
test('lifecycle and export download use provider methods', async () => {
  ...
  store.state.documentUploads.push({
    ...
    status: 'uploaded',
    uploadedBy: 'advisor',
    object: { bucket: 'test-docs', key: 'old-object', checksum: null, contentType: 'application/pdf', retentionClass: 'uploaded_document' },
