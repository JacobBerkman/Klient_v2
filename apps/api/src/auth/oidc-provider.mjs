...
function normalizeClaims(rawClaims) {
  ...
  const email = normalizeEmail(pickFirst(claims, ['email', 'preferred_username', 'upn', 'mail']));
  const subject = String(pickFirst(claims, ['sub', 'subject', 'id', 'user_id']) || '').trim();
  if (!email) throw new Error('OIDC claims missing email.');
  if (!subject) throw new Error('OIDC claims missing subject.');
  return {
    subject,
    email,
    firstName: String(pickFirst(claims, ['given_name', 'first_name', 'firstName']) || '').trim() || 'Unknown',
    lastName: String(pickFirst(claims, ['family_name', 'last_name', 'lastName']) || '').trim() || 'User',
    organization: String(pickFirst(claims, ['org_name', 'organization', 'tenant', 'company']) || '').trim() || null,
...
function createFederatedProvider({
  ...
  function mapIdentity(claims, input = {}) {
    if (!state || typeof createSession !== 'function') throw new Error(`${provider.toUpperCase()} provider requires state and createSession.`);
    ensureArrays(state);

    const normalized = normalizeClaims(claims);
    const identity = state.externalIdentities.find((entry) => entry.provider === provider && entry.subject === normalized.subject);
    ...
    if (!user) {
      ...
      if (!firmId) {
        const firmName = input?.firmName || normalized.organization || `${normalized.firstName} ${normalized.lastName} Advisory`;
        const firm = {
          id: randomUUID(),
          name: firmName,
  ...
  return {
    ...
    createMfaChallenge(user) {
      if (typeof hooks.createMfaChallenge !== 'function') return { ok: true, providerManaged: true, provider, methods: ['provider'] };
...
export function createOidcAuthProvider(options = {}) {
  return createFederatedProvider({ provider: 'oidc', ...options });
}
