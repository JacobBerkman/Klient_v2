...
test('forms draft sharing contract exposes panel controls, live-region feedback, and role gating rules', () => {
  ...
  assert.match(appJs, /Membership refreshed from server\./)
  ...
  assert.match(appJs, /Search complete: \${state\.formsUi\.userLookupByDraftId\[draftId\]\.length} candidate user\(s\) ready to add\./)
  assert.match(appJs, /Search complete: no candidates found\. Refine your query and retry\./)
  assert.match(appJs, /Add complete: \${collaboratorIdentityLabel\(userId\)} now has draft access\. Membership is current\./)
  assert.match(appJs, /Remove complete: \${collaboratorIdentityLabel\(/)
