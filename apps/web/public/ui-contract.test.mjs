...
test('custom field admin and profile edit flows expose typed affordances, accessibility semantics, and failure/empty-state recovery', () => {
  ...
  assert.match(appJs, /custom-field-admin-save-status/)
  assert.match(appJs, /Saving custom field schema changes…/)
  assert.match(appJs, /Saved at \$\{escapeHtml\(new Date\(saveStatus\.savedAt \|\| Date\.now\(\)\)\.toLocaleTimeString\(\)\)\}\./)
  assert.match(appJs, /data-custom-field-retry-last-save/)
  assert.match(appJs, /Retry last save/)
  assert.match(appJs, /saveStatus\.conflictHint/)
  assert.match(appJs, /validation-summary/)
  assert.match(appJs, /data-validation-jump/)
  assert.match(appJs, /aria-describedby', `\$\{targetId\}-error`/)
  assert.match(appJs, /data-custom-field-unsaved-badge/)
  assert.match(appJs, /Unsaved changes/)
  assert.match(appJs, /All changes saved/)
  assert.match(appJs, /Create first field/)
  assert.match(appJs, /Expected outcome: new fields appear on profile create\/edit forms/)
...
test('styles provide visible focus indicators and empty-state affordances', () => {
  ...
  assert.match(styles, /\.validation-summary \{/)
  assert.match(styles, /\.badge\.success \{/)
