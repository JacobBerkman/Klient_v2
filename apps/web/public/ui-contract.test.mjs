...
test('custom field admin and profile edit flows expose typed affordances, accessibility semantics, and failure/empty-state recovery', () => {
  ...
  assert.match(appJs, /Preview generated\. Confirm to persist changes\./)
  assert.match(appJs, /Bulk schema changes saved\./)
  assert.match(appJs, /parseBulkRows/)
  assert.match(appJs, /Bulk edit contains validation errors\. Fix highlighted rows and retry\./)
  ...
  assert.match(appJs, /Loading custom field schema…/)
  assert.match(appJs, /Could not load schema:/)
  assert.match(appJs, /retry-custom-field-schema/)
  assert.match(appJs, /open-custom-fields-view/)
  assert.match(appJs, /Profile creation can continue without custom fields, or retry now\./)
  assert.match(appJs, /Create fields to capture operator-specific values in profile and draft workflows\./)
  assert.match(appJs, /Freeform text value\./)
  ...
  assert.match(appJs, /Use retry to attempt save again\./)
