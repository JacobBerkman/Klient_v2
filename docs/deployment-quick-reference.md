...
| Queue backlog growth / stalled processing       | `queue.pending`, `queue.stalled`, `queue.readyNow`, `queue.activeLeasesCount`, `queue.workerObservedRecently`, `queue.pendingWithoutWorker`                  | `postdeploy-exports-queue.json` (`/api/ops/exports/queue`)               | Companion worker is not draining jobs fast enough, is not heartbeating, or lease contention exists. | Verify the companion export worker is running; use the manual process endpoint only as a recovery tick; hold GO decision until queue stabilizes. |
