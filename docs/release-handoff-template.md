...
- Include linkage contract fields from `details.artifacts.playwrightEvidenceLinkage`: canonical evidence-dir-relative `reportPath`, strict-mode `provisioningArtifactPath`, and optional absolute mirrors (`reportPathAbsolute`, `provisioningArtifactPathAbsolute`) only when they resolve to the same canonical files.
