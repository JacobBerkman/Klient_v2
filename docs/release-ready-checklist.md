...
| E2E UI contract checks | QA Lead | `npm run test:e2e` | `artifacts/release-evidence/<release-id>/e2e-summary.json` | Exit code `0`; summary has `status=passed`. | **SEV-1** | Block GO/NO-GO if E2E UI contract assertions fail or evidence is missing. |
