...
| E2E browser checks | QA Lead | `npm run test:e2e` | `artifacts/release-evidence/<release-id>/e2e-summary.json` | Exit code `0`; summary has `status=passed`; browser run metadata (base URL + Playwright report details) is present. | **SEV-1** | Roll back if core sign-in/dashboard journey fails in production or E2E evidence is missing/inconclusive. |
