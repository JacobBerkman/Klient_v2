...
| Advisor workflow completion (template detail preview + publish)                                                                          | `npx playwright test tests/e2e/workflows.spec.mjs --grep "routed template detail supports preview and publish controls"` | Deterministically covers routed template detail/editor behavior in the React shell.                                                                            |
| Portal submit lifecycle (draft -> submitted)                                                                                             | `npx playwright test tests/e2e/workflows.spec.mjs --grep "routed portal token lifecycle saves draft and submits form"`   | Explicitly verifies the routed `/portal` submission lifecycle in the React shell.                                                                              |
| Release smoke journey (health/ready + PDF template auto-build + generated form submission + companion-worker PDF/XLSX exports/downloads) | `npm run test:smoke`                                                                                                     | Validates the canonical production smoke path, including persisted template/export artifacts, worker processing, and release evidence in the canonical schema. |
